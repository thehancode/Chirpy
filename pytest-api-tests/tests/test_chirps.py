...
def test_create_chirp(client,auth_headers, base_url, user):
    ...
    response = client.post(url, json=chirp_data, headers=auth_headers)
    assert response.status_code == 201
    chirp = response.json()
    assert 'id' in chirp

def test_get_all_chirps(client, base_url):
...
def test_get_chirp_by_id_success(client,auth_headers, base_url, user):
    ...
    create_response = client.post(create_url, json=chirp_data, headers=auth_headers)
    ...
    assert chirp['user_id'] ==user["id"], "Chirp user_id does not match"
...
def test_get_chirp_by_id_not_found(client, auth_headers, base_url):
...
def test_get_chirp_by_id_invalid_uuid(client, auth_headers, base_url):
